...
router.get('/', optionalAuthenticateToken, async (req, res) => {
    try {
        ...
        if (userId) {
            ...
            const booksWithPersonalization = await Promise.all(books.map(async (book) => {
...
router.get('/:idOrSlug', optionalAuthenticateToken, async (req, res) => {
    try {
...
router.post('/', authenticateToken, requireRole('ADMIN'), async (req, res) => {
    try {
        const { title, description, shortDescription, author, category, publicationYear, isbn, language, readingLevel, coverImage, thumbnail, featured, published, slug: customSlug, qrLogo, readingTime, } = req.body;
...
router.patch('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const { title, description, shortDescription, author, category, publicationYear, isbn, language, readingLevel, coverImage, thumbnail, featured, published, slug: newSlug, qrLogo, readingTime, } = req.body;
        const updateData = {};
        if (title)
        ...
        if (publicationYear !== undefined)
            updateData.publicationYear = publicationYear ? parseInt(publicationYear, 10) : null;
        if (isbn !== undefined)
