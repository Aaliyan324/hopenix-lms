...
import { authenticateToken, requireRole, requireCourseReadAccess, AuthenticatedRequest } from '../middleware/auth.js';
...
router.get('/:id', authenticateToken, requireCourseReadAccess, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { role, userId } = req.user!;
...
// Update Course (ADMIN)
router.patch('/:id', authenticateToken, requireRole('ADMIN'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.get('/:id/qr', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const course = await prisma.course.findFirst({
      where: { OR: [{ id }, { slug: id }] },
