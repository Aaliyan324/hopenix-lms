...
import { authenticateToken, optionalAuthenticateToken, requireRole, requireLessonEditPermission, } from '../middleware/auth.js';
...
router.get('/editor/assigned', authenticateToken, requireRole('EDITOR'), async (req, res) => {
    try {
...
router.patch('/:id', authenticateToken, requireLessonEditPermission, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (lessonNumber !== undefined)
            updateData.lessonNumber = parseInt(lessonNumber, 10);
...
router.post('/reorder', authenticateToken, requireRole('ADMIN'), async (req, res) => {
    try {
        ...
        await Promise.all(items.map((item, index) => prisma.lesson.update({
            where: { id: item.id },
            data: {
...
router.get('/:id/editors', authenticateToken, requireRole('ADMIN'), async (req, res) => {
    try {
        const { id } = req.params;
        const permissions = await prisma.lessonEditorPermission.findMany({
            where: { lessonId: id },
...
router.post('/:id/editors', authenticateToken, requireRole('ADMIN'), async (req, res) => {
    try {
        const { id } = req.params;
