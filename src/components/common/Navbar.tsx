...
export const Navbar: React.FC<NavbarProps> = ({ onToggleSidebar }) => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/90 border-b border-slate-200/80 backdrop-blur-md transition-all shadow-xs">
      <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-4 sm:gap-8">
          ...
          <Link to={getHomeLink()} className="flex items-center gap-3 group">
            <div className="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center text-orange-500 shadow-book group-hover:bg-slate-800 transition-all duration-300 group-hover:scale-105">
              <BookOpen className="w-5 h-5 fill-current" />
            </div>
            <div className="flex flex-col">
              <span className="font-serif font-bold text-xl text-slate-900 tracking-tight leading-none">
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <div className="relative">
              <motion.button
                ...
                className="flex items-center gap-3 p-1.5 pl-3 rounded-2xl bg-slate-50 hover:bg-slate-100/80 transition-all border border-slate-200/80 group"
              >
                <img
                  src={user.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.name}`}
                  alt={user.name}
                  className="w-8 h-8 rounded-xl bg-slate-200 border border-slate-300 object-cover shadow-2xs"
                />
                ...
                {dropdownOpen && (
                  <>
                    ...
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-editorial z-20 overflow-hidden py-1.5"
                    >
                      ...
                      <div className="py-2 space-y-0.5 px-2">
                        {isEditor && (
                          <Link
                            ...
                            className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl text-xs font-semibold text-slate-700 hover:bg-orange-50 hover:text-orange-700 transition-colors"
                          >
          ...
          <motion.button
            ...
            className="md:hidden p-2 rounded-xl text-slate-600 hover:text-slate-900 bg-slate-50 border border-slate-200 transition-colors"
          >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden border-t border-slate-200 bg-white px-4 py-4 space-y-2 overflow-hidden shadow-lg"
          >
            {isEditor && (
              <Link
                to="/editor"
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-semibold text-slate-800 hover:bg-orange-50 hover:text-orange-700 transition-colors"
              >
