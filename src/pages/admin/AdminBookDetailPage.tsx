...
export const AdminBookDetailPage: React.FC = () => {
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <a href={`/books/${book.slug}`} target="_blank" rel="noreferrer">
              <button className="inline-flex items-center gap-2 px-4 py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold rounded-2xl transition-all shadow-lg shadow-orange-500/25">
      ...
      <form onSubmit={handleUpdateBook} className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 space-y-6 shadow-editorial hover:shadow-editorial transition-shadow">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          ...
          <button
            type="submit"
            disabled={savingBook}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold rounded-2xl transition-all shadow-md shadow-orange-500/25 disabled:opacity-50"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 focus:border-orange-500 rounded-xl px-3 py-2.5 text-sm text-slate-900 outline-none transition-colors"
              required
            />
          </div>
          <div>
        ...
        <div className="relative">
          <Building2 className="w-3.5 h-3.5 text-slate-400 absolute left-3 top-8 pointer-events-none" />
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Company / Publisher Name</label>
            <input
              type="text"
              placeholder="e.g. Hopenix Inc., Acme Corp"
              value={companyName}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCompanyName(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 focus:border-orange-500 rounded-xl px-3 py-2.5 text-sm text-slate-900 outline-none transition-colors pl-9"
            />
          </div>
        </div>

        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-600">Book Overview / Description</label>
          <textarea
            rows={4}
            value={description}
            onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setDescription(e.target.value)}
            className="w-full bg-slate-50 border border-slate-200 focus:border-orange-500 rounded-xl p-3 text-sm text-slate-900 outline-none transition-colors"
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-sm">
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="font-semibold text-slate-600">CLASS / GRADE</label>
              <button
                type="button"
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Published Year</label>
            <input
              type="text"
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          />
          <label htmlFor="pub-check" className="text-sm font-semibold text-slate-700 cursor-pointer">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold rounded-2xl transition-all shadow-md shadow-orange-500/25"
          >
        ...
        <div className="space-y-3">
          {book.lessons?.map((lesson, idx) => {
            ...
            return (
              <div
                key={lesson.id}
                className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white border border-slate-200/80 rounded-2xl hover:border-orange-300 hover:shadow-editorial transition-all group"
              >
                <div className="flex items-center gap-4">
                  <span className="w-9 h-9 rounded-xl bg-orange-50 border border-orange-200 flex items-center justify-center font-bold text-sm text-orange-700">
                    L{num}
                  </span>
                ...
                <div className="flex items-center gap-2 mt-3 sm:mt-0">
                  <Badge variant={lesson.published ? 'success' : 'warning'} size="sm" className={lesson.published ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}>
                    {lesson.published ? 'Published' : 'Draft'}
                  </Badge>

                  <button
                    className="p-2 text-slate-600 hover:text-orange-600 hover:bg-orange-50 rounded-xl transition-all border border-slate-200 hover:border-orange-300"
                    onClick={() => {
                  ...
                  <Link to={`/admin/lessons/${lesson.id}/edit`}>
                    <button className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 hover:bg-orange-50 text-slate-700 hover:text-orange-700 text-xs font-semibold rounded-xl border border-slate-200 hover:border-orange-300 transition-all">
      ...
      <Modal
        ...
        maxWidth="lg"
      >
        <form onSubmit={handleCreateLesson} className="space-y-5 text-sm">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">Lesson Number</label>
              <input
                type="number"
                value={lessonNumber}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Rich Text Content</label>
          ...
          <div className="flex items-center gap-2 pt-2">
            <input
              ...
              className="w-4 h-4 rounded border-slate-300 text-orange-500 focus:ring-orange-500 bg-slate-50 cursor-pointer"
            />
            <label htmlFor="l-pub-check" className="text-sm font-semibold text-slate-700 cursor-pointer">
